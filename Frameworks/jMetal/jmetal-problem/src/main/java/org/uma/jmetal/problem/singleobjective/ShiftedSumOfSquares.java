...
public class ShiftedSumOfSquares  extends LoggingDoubleProblem {
    ...
    public ShiftedSumOfSquares() {
        this(10);
    }

    public ShiftedSumOfSquares(Integer numberOfVariables) {
        numberOfObjectives(1);
        name("ShiftedSumOfSquares");
    ...
    public DoubleSolution computeFitness(DoubleSolution solution) {
        ...
        for (int i = 0; i < numberOfVariables(); i++) {
            shiftedX[i] = x.get(i) - shift[i];
        }

        for (int i = 0; i < numberOfVariables(); i++) {
            fitness += (i + 1) * pow(shiftedX[i], 2);
        }
