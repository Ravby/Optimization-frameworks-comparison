...
static const double shiftedRastrigin[]
...
rastriginShifted::rastriginShifted(unsigned dim) : m_dim(dim)
{
    if (dim < 1u) {
        pagmo_throw(std::invalid_argument,
                    "rastriginShifted Function must have minimum 1 dimension, " + std::to_string(dim) + " requested");
    }
}
 ...
 */
vector_double rastriginShifted::fitness(const vector_double &x) const
{
    vector_double f(1, 0.);
    ...
    for (int i = 0; i < n; ++i) {
        shifted[i] = x[i] - shiftedRastrigin[i];
    }
 ...
 */
std::pair<vector_double, vector_double> rastriginShifted::get_bounds() const
{
 ...
 */
vector_double rastriginShifted::gradient(const vector_double &x) const
{
    auto n = x.size();
    vector_double g(n);
    const auto omega = 2. * pagmo::detail::pi();
 ...
 */
std::vector<vector_double> rastriginShifted::hessians(const vector_double &x) const
{
    auto n = x.size();
    vector_double h(n);
    const auto omega = 2. * pagmo::detail::pi();
 ...
 * @return the hessians of the fitness function
 */
std::vector<sparsity_pattern> rastriginShifted::hessians_sparsity() const
{
    sparsity_pattern hs;
    auto n = m_dim;
    for (decltype(n) i = 0u; i < n; ++i) {
 ...
 */
vector_double rastriginShifted::best_known() const
{
...
void rastriginShifted::serialize(Archive &ar, unsigned)
{
    ar &m_dim;
}
...
PAGMO_S11N_PROBLEM_IMPLEMENT(pagmo::rastriginShifted)
