...
static const double shiftedSchwefel[]
...
schwefelShifted::schwefelShifted(unsigned dim) : m_dim(dim)
{
    if (dim < 1u) {
        pagmo_throw(std::invalid_argument,
                    "schwefelShifted Function must have minimum 1 dimension, " + std::to_string(dim) + " requested");
    }
}
 ...
 */
vector_double schwefelShifted::fitness(const vector_double &x) const
{
    vector_double f(1, 0.);
    auto n = x.size();
    double sum = 0.;
    ...
    for (int i = 0; i < n; ++i) {
        shifted[i] = x[i] - shiftedSchwefel[i];
    }
 ...
 */
std::pair<vector_double, vector_double> schwefelShifted::get_bounds() const
{
 ...
 */
vector_double schwefelShifted::best_known() const
{
...
void schwefelShifted::serialize(Archive &ar, unsigned)
{
    ar &m_dim;
}
...
PAGMO_S11N_PROBLEM_IMPLEMENT(pagmo::schwefelShifted)
