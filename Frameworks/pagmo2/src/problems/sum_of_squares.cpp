...
sumOfSquares::sumOfSquares(unsigned dim) : m_dim(dim)
{
    if (dim < 1u) {
 ...
 */
vector_double sumOfSquares::fitness(const vector_double &x) const
{
    vector_double f(1, 0.);
    auto n = x.size();
 ...
 */
std::pair<vector_double, vector_double> sumOfSquares::get_bounds() const
{
 ...
 */
vector_double sumOfSquares::best_known() const
{
...
void sumOfSquares::serialize(Archive &ar, unsigned)
{
    ar &m_dim;
}
...
PAGMO_S11N_PROBLEM_IMPLEMENT(pagmo::sumOfSquares)
