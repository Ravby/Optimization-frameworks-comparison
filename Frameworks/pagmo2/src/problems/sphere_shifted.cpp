...
static const double shiftedSphere[]
...
sphereShifted::sphereShifted(unsigned dim) : m_dim(dim)
{
    if (dim < 1u) {
 ...
 */
vector_double sphereShifted::fitness(const vector_double &x) const
{
    vector_double f(1, 0.);
    auto n = x.size();
 ...
 */
std::pair<vector_double, vector_double> sphereShifted::get_bounds() const
{
 ...
 */
vector_double sphereShifted::best_known() const
{
...
void sphereShifted::serialize(Archive &ar, unsigned)
{
    ar &m_dim;
}
...
PAGMO_S11N_PROBLEM_IMPLEMENT(pagmo::sphereShifted)
