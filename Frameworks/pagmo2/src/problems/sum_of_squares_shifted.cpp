...
static const double sosShifted[]
...
sumOfSquaresShifted::sumOfSquaresShifted(unsigned dim) : m_dim(dim)
{
    if (dim < 1u) {
 ...
 */
vector_double sumOfSquaresShifted::fitness(const vector_double &x) const
{
    vector_double f(1, 0.);
    auto n = x.size();
    ...
    for (int i = 0; i < n; ++i) {
        shifted[i] = x[i] - sosShifted[i];
    }


    for (decltype(n) i = 0u; i < n; i++) {
        f[0] += i * shifted[i] * shifted[i];
    }
 ...
 */
std::pair<vector_double, vector_double> sumOfSquaresShifted::get_bounds() const
{
 ...
 */
vector_double sumOfSquaresShifted::best_known() const
{
...
void sumOfSquaresShifted::serialize(Archive &ar, unsigned)
{
    ar &m_dim;
}
...
PAGMO_S11N_PROBLEM_IMPLEMENT(pagmo::sumOfSquaresShifted)
