...
namespace pagmo
{
static const double shiftedGriewank[]
...
griewankShifted::griewankShifted(unsigned dim) : m_dim(dim)
{
    if (dim < 1u) {
        pagmo_throw(std::invalid_argument,
                    "griewankShifted Function must have minimum 1 dimension, " + std::to_string(dim) + " requested");
    }
}
 ...
 */
vector_double griewankShifted::fitness(const vector_double &x) const
{
    vector_double f(1, 0.);
    auto n = x.size();
    double fr = 4000.;
    ...
    for (int i = 0; i < n; ++i) {
        shifted[i] = x[i] - shiftedGriewank[i];
    }
 ...
 */
std::pair<vector_double, vector_double> griewankShifted::get_bounds() const
{
 ...
 */
vector_double griewankShifted::best_known() const
{
...
void griewankShifted::serialize(Archive &ar, unsigned)
{
    ar &m_dim;
}
...
PAGMO_S11N_PROBLEM_IMPLEMENT(pagmo::griewankShifted)
